...
public class ProductDAO {
	...
	public static void main(String[] args) {
		System.out.println(getProductsByCategoryId(1, 1, 2));
	}
	...
	public static ArrayList<Product> getProductsByCategoryId(int id, int page, int perpage) {
		...
		String sql = "SELECT * FROM products WHERE category_id = ? OR ? = 0 LIMIT ?,?;";
	...
	public static boolean insert(Product product) {
		String sql = "INSERT INTO `products`(`name`, `price`, `detail`, `origin`, `image`, `category_id`) VALUES (?,?,?,?,?,?)";
		...
		try {
			...
			p.setString(4, product.getOrigin());
			FileInputStream fis;
			try {
			...
			p.setBinaryStream(5, (InputStream)fis);
			p.setInt(6, product.getCategory_id());
	...
	public static boolean update(Product product) {
		String sql = "UPDATE `products` SET `name`=?,`price`=?,`detail`=?, `origin`=?,`category_id`=? WHERE id = ?";
